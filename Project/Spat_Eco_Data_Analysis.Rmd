...
covs <- PresBackSegs %>% dplyr::select(CATAREASQKM:TMEAN8110CAT)
...
nFolds = 4
kfoldPres = kfold(presSegs, k=nFolds)
kfoldBack = kfold(backSegs, k=nFolds)
...
for(i in 1:nFolds){
  ...
  mutate(obs = valPres$pres,
         MEVal = predict(Model2, valPres %>% select(canopy:precip), type='response'))
