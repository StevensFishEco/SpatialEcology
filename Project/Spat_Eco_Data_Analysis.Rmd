...
St0101var <- sc_get_data(comid = St0101$COMID, aoi='catchment', metric='BFI,Precip8110,Elev,fert,HUDen2010')

St0101f <- merge(St0101, St0101var, by = "COMID")


St0102var <- sc_get_data(comid = St0102$COMID, aoi='catchment', metric='BFI,Precip8110,Elev,fert,HUDen2010')
...
EnvCor <- data.frame(catchmentarea = EnvFull$CATAREASQKM, Elevation = EnvFull$ELEVCAT, BFI = EnvFull$BFICAT, HumanDen = EnvFull$HUDEN2010CAT, Precipitation = EnvFull$PRECIP8110CAT, Fertilizer = EnvFull$FERTCAT)
...
backSegs <- dplyr::slice_sample(EnvFull %>% filter(!COMID %in% presSegs$COMID), n = 200)
...
covs <- PresBackSegs %>% dplyr::select(CATAREASQKM:PRECIP8110CAT)
...
Suitability = predictMaxNet(maxentModel, as.data.frame(EnvFull), type='logistic')
...
projected <- cbind(EnvFull, Suitability)
...
for(i in 1:nFolds){
  ...
  mutate(obs = valPres$Present,
         MEVal = predict(Model2, valPres %>% dplyr::select(CATAREASQKM:PRECIP8110CAT), type='logistic'))
  
boyceVals[i] = ecospat.boyce(fit = maxentMap, obs=valData[,4], res=100, PEplot=F)$cor
