...
siteData = read.csv("Proposed_SUR_Locations.csv")
```
...
Basin <- st_read("Shape/WBDHU6.shp")
...
Rivers <- st_read("Shape/NHDFlowline.shp")

# needed to remove Z and M ranges from rivers shapefle
Rivers = st_zm(Rivers)
...
# read in an extra file for important unnamed stream segments (separate file because I had to hand delineate these segments instead of a simply query selection used in ArcMap for named segments)
Segments <- st_read("Shape/NHDArea.shp")

# needed to remove Z and M ranges from rivers shapefle
Segments = st_zm(Segments)

# looking good on my end
plot(st_geometry(Segments))

# whats the projection
st_crs(Segments)$proj4string

# read in lakes 
Lakes <- st_read("Shape/NHDWaterbody.shp")

# needed to remove Z and M ranges from lakes shapefle
Lakes = st_zm(Lakes)
...
ggplot()+
  ...
  geom_sf(data = st_geometry(Segments), color = "darkblue") +
...
crs(Segments, proj = T)
Segments = st_transform(Segments, st_crs(mississippi))
crs(Segments, proj = T)
...
zoomedIn = zoomedIn +
  ...
  geom_sf(data = st_geometry(Segments), color = "darkblue", fill = "darkblue") +
